...
def predict_batch(images, categories, clip_model_gelu, clip_model_convnext, model_gelu, model_convnext, checkpoint_gelu, checkpoint_convnext, clip_preprocess_gelu, clip_preprocess_convnext, device='cuda', batch_size=32):
    ...
    with torch.no_grad():
        ...
        for batch_images, batch_categories in pbar:
            ...
            with torch.autocast(device):
                ...
                clip_features_convnext = clip_model_convnext.encode_image(batch_images_convnext)
                ...
                for idx, category in enumerate(batch_categories):
                    ...
                    predictions_gelu = model_gelu(clip_features_gelu[idx:idx+1], category)
                    predictions_convnext = model_convnext(clip_features_convnext[idx:idx+1], category)
...
def load_models(model_path_gelu, model_path_convnext, device,cache_dir=None):
    ...
    clean_clip_checkpoint_gelu = clean_state_dict(checkpoint_gelu['clip_model_state_dict'])
    ...
    clean_clip_checkpoint_convnext = clean_state_dict(checkpoint_convnext['clip_model_state_dict'])
    
    clip_model_convnext, clip_preprocess_convnext = create_clip_model_convnext('cpu',cache_dir)
    ...
    model_gelu = CategoryAwareAttributePredictor(
        ...
        attribute_dims={key: len(values) for key, values in checkpoint_gelu['dataset_info']['attribute_classes'].items()},
    ...
    model_convnext = CategoryAwareAttributePredictor(
        ...
        attribute_dims={key: len(values) for key, values in checkpoint_convnext['dataset_info']['attribute_classes'].items()},
    ...
    clean_cat_checkpoint_convnext = clean_state_dict(checkpoint_convnext['model_state_dict'])
    ...
    return model_gelu, clip_model_gelu, clip_preprocess_gelu, checkpoint_gelu['dataset_info'], model_convnext, clip_model_convnext, clip_preprocess_convnext, checkpoint_convnext['dataset_info']


def process_csv_file(input_csv_path, image_dir, model_path_gelu, model_path_convnext, output_csv_path, batch_size=32, device='cuda', cache_dir=None):
    # Load the input CSV
    ...
    model_gelu, clip_model_gelu, clip_preprocess_gelu, checkpoint_gelu, model_convnext, clip_model_convnext, clip_preprocess_convnext, checkpoint_convnext = load_models(model_path_gelu, model_path_convnext, device,cache_dir)
...
def parse_args():
    ...
    parser.add_argument('--model_path_convnext',
                       type=str,
                       help='Path to CONVNEXT model checkpoint')
    
    parser.add_argument('--model_path_gelu',
                          type=str,
                          help='Path to GELU model checkpoint')
